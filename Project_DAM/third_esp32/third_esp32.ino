...
#define WIFI_SSID "Redmi"                            // "MEO-CFA840"
#define WIFI_PASSWORD "12345678"                       // "cc3c2f82a9"

// Insert Firebase project API Key
#define API_KEY "AIzaSyDzWeLuRMJvxzNfAeuYLI1wpk1yqwciadM"

// Insert RTDB URLefine the RTDB URL */
#define DATABASE_URL "https://dam20222023normal-d7271-default-rtdb.europe-west1.firebasedatabase.app/" 

unsigned long sendDataPrevMillis = 0;
unsigned long timerDelay = 15000;     // 180000
...
int get_daynight_info(){
  
  int day_night_info = -1;

  if (Firebase.RTDB.getInt(&fbdo, "/rulesChicken/day_night")) {   // change and complete
      if (fbdo.dataType() == "int") {
        day_night_info = fbdo.intData();
...
void IRAM_ATTR detectsMovement_first() {
  ...
  json.set("/ESPChicken/sensors/move_left_sec", "ON");   
...
void IRAM_ATTR detectsMovement_second() {
  ...
  Firebase.RTDB.setString(&fbdo, "/ESPChicken/sensors/move_right_sec", "ON");

 // json.set("/ESPChicken/sensors/move_right_sec", "ON");
...
void loop() {
  ...
  //  int * control_data_third = read_all_states();
    ...
    int control_data_third[] = {1,1,1};

    int day_night_bit = get_daynight_info();
    ...
    //    }
      ...
      if(control_data_third[0] != -100){  
        if(detected_move_first == true){
          String msg_off = "OFF";   
          json.set("/ESPChicken/sensors/move_left_sec", "OFF");
      ...
      if(control_data_third[1] != -100){  
        if(detected_move_second == true){
          String msg_off = "OFF";  
          json.set("/ESPChicken/sensors/move_right_sec", "OFF");
