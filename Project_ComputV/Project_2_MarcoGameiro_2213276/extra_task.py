# -*- coding: utf-8 -*-
"""
Created on Tue Dec 27 16:27:52 2022
...
def background_subtraction_chess_seg(img_mask, bin_image):       ## , dir_ref_img 
    ...
    from control_exec import camera_calib_opt
    
    opt_cam = camera_calib_opt()
...
##                                  ((int(len(got_img[0])/2 + len(roi_chess[0])/2), int(len(got_img)/2 + len(roi_chess)/2))), color, thickness)
    ...
    if imx is None:
        print("Test image example giving none")
...
if type(newcameramtx) == int:
    ...
    metadata_csv_filename = dir_rnew + meta_title_info + getDateTimeStrMarker()
    ...
    proc_time = t1_stop - t1_start
    
    print("Processing time for calibration task was about " + str(proc_time) + " seconds")
    
    
    # cv.imwrite("C:\\VisComp\\PL\\Project_2\\Data\\Im_L_1.png", imx)
    # Out[123]: True
    ...
    #     for i in range(3,12):
    #         if i != j:
    #             found, corners = cv.findChessboardCorners(imx, (i,j))
    #             if found:
    #                 print("Worked")
    #                 i_worked = i
    #                 j_worked = j
